...
-(void)drawAnimated:(BOOL)animated{
    ...
    CALayer* line = [BaseLayer layerOfLineFrom:CGPointZero to:CGPointMake(0, self.designHight+1) withColor:[BBTheme defTheme].axisColor andWidth:1.5];
    ...
    CGFloat labelHei = [self sizeOfText:@"abc" andSize:[BBTheme defTheme].yAxisFontSize].height;

    for (int i = 1; i < cnt; ++i) {
        CGFloat curHei = i * labelGap;
        CALayer* dash = [BaseLayer layerOfLineFrom:CGPointMake(self.bounds.size.width-1.5-5, curHei) to:CGPointMake(self.bounds.size.width-2, curHei) withColor:[BBTheme defTheme].axisColor andWidth:1];
        ...
        CATextLayer* t = [BaseLayer layerOfText:lab withFont:@"Helvetica" fontSize:[BBTheme defTheme].yAxisFontSize andColor:[BBTheme defTheme].axisColor];
